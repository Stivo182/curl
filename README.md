...
#### 1. Использование в составе конфигурации/расширениии
...
> [!IMPORTANT]
> Если в переменных среды операционной системы отсутствует `curl`, то в начале необходимо вызвать метод `УказатьИсполняемыйФайл` с указанием пути исполняемому файлу
> ```bsl
...
```bsl
Текст = CURL.Post("https://example.com/v1/api").ОтветКакТекст();
```

#### Обработка результата как десериализованный Json

```bsl
Json = CURL.Post("https://example.com/v1/api").ОтветКакJson();
```

#### Обработка результата как двоичные данные

```bsl
ДвоичныеДанные = CURL.Post("https://example.com/v1/api").ОтветКакДвоичныеДанные();
```

#### Обработка результата как путь к файлу

```bsl
ПутьКФайлу = CURL.Post("https://example.com/v1/api").ОтветКакПутьФайлу();
```
...
```bsl
ЗаголовкиОтвета = CURL.Get("https://example.com/").ЗаголовкиОтвета();
```
...
Получение новых куки полученые из заголовка ответа `Set-Cookie`:
```bsl
...
Использование шифров с соединении:
```bsl
