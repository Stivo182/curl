...
Перем ВременныеФайлы; // Соответствие
...
Процедура ЗаполнитьНаборТестов(НаборТестов, КонтекстЯдраПараметр) Экспорт
	...
	#Область Post
		...
		НаборТестов.Добавить("ТестДолжен_ПроверитьНевозможностьПередачиДанныхОдновременноСMultipart");
	...
	#Область Заголовки
		...
		НаборТестов.Добавить("ТестДолжен_ПроверитьВыполнениеМетодаПолученияЗаголовков");
	#КонецОбласти
	...
	#Область Сертификаты
		...
		НаборТестов.Добавить(
			"ТестДолжен_ПроверитьВСтрокеКомандыИспользованиеСертификатКлиентаСПаролем");
		НаборТестов.Добавить(
	...
	#Область Прокси
		...
		НаборТестов.Добавить("ТестДолжен_ПроверитьВСтрокеКомандыИспользованиеНезащищенногоПодключенияКПрокси");	
		
		#Область Сертификаты
			НаборТестов.Добавить(
				"ТестДолжен_ПроверитьВСтрокеКомандыИспользованиеСертификатКлиентаПрокси");
			НаборТестов.Добавить(
				"ТестДолжен_ПроверитьВСтрокеКомандыИспользованиеСертификатКлиентаПроксиСПаролем");
			НаборТестов.Добавить(
	...
	#Область Прочее
		...
		НаборТестов.Добавить("ТестДолжен_ПроверитьНаличиеВКомандеЗапросаНаСжатиеОтвета");
		НаборТестов.Добавить("ТестДолжен_ПроверитьВСтрокеКомандыИспользованиеНезащищенногоПодключения");
		НаборТестов.Добавить("ТестДолжен_ПроверитьОтсутствиеПеренаправленияВРежимеЭмуляцииБраузера");
		НаборТестов.Добавить("ТестДолжен_ПроверитьНаличиеПеренаправленияВРежимеЭмуляцииБраузера");
	#КонецОбласти
...
Процедура ТестДолжен_СкачатьФайл() Экспорт
...
Процедура ТестДолжен_СкачатьФайлСУказаниемМестаСохранения() Экспорт
	
	Размер = 16;
	//@skip-check missing-temporary-file-deletion - Временный файл удаляется далее процедурой Curl.Сбросить()
	ПутьКФайлу = ПолучитьИмяВременногоФайла("bin");
	URL = СтрШаблон("https://httpbin.org/bytes/%1", Формат(Размер, "ЧГ="));
	
	Curl.СкачатьФайл(URL, ПутьКФайлу);
...
Процедура ТестДолжен_ПроверитьПередачуСодержимогоMultipart() Экспорт
	...
	// Файл как путь к строке
	ПутьКФайлу = НовоеИмяВременногоФайла();
	ДвоичныеДанные.Записать(ПутьКФайлу);
	
	// Файл как двоичные данные во временном хранилище
	АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанные);
	
	// Текст из файла
	Поток = Новый ПотокВПамяти();
	ЗаписьТекста = Новый ЗаписьТекста(Поток, , , , Ложь);
	ЗаписьТекста.Записать("Длинный текст");
	ЗаписьТекста.Закрыть();
	ПутьКТекстовомуФайлу = НовоеИмяВременногоФайла();
	Поток.ЗакрытьИПолучитьДвоичныеДанные().Записать(ПутьКТекстовомуФайлу);
	
	Curl
		.ДобавитьТекстMultipart("Name", "Tom", "text/plain")
		.ДобавитьТекстMultipart("Age", "45")
		.ДобавитьТекстИзФайлаMultipart("LongText", ПутьКТекстовомуФайлу)
	...
	Ожидаем.Что(ДанныеФормы.Получить("LongText")).Равно("Длинный текст");
...
Процедура ТестДолжен_ПроверитьНевозможностьПередачиДанныхОдновременноСMultipart() Экспорт
	
	Curl.ДобавитьТекстMultipart("Name", "Tom");
...
Процедура ТестДолжен_ПроверитьВСтрокеКомандыИспользованиеПеренаправления() Экспорт
	...
	СтрокаКоманды = Curl
		.РазрешитьПеренаправления()
		.УстановитьМаксимальноеКоличествоПеренаправлений(МаксимальноеКоличествоПеренаправлений)
...
Процедура ТестДолжен_ПроверитьВыполнениеМетодаПолученияЗаголовков() Экспорт

	Заголовки = Curl.ПолучитьЗаголовки("https://httpbin.org/get");
...
Процедура ТестДолжен_ПроверитьВСтрокеКомандыИспользованиеСертификатКлиента() Экспорт
	...
	Опция = ПодготовитьОпцию("--cert", ИмяФайлаСертификата);

	Curl.УстановитьСертификатКлиента(ИмяФайлаСертификата);
...
Процедура ТестДолжен_ПроверитьВСтрокеКомандыИспользованиеСертификатКлиентаСПаролем() Экспорт
...
Процедура ТестДолжен_ПроверитьВСтрокеКомандыИспользованиеНезащищенногоПодключенияКПрокси() Экспорт
	
	Опция = "--proxy-insecure";

	СтрокаКоманды = Curl
		.УстановитьПрокси("https", "127.0.0.1")
...
Процедура ТестДолжен_ПроверитьВСтрокеКомандыИспользованиеСертификатКлиентаПрокси() Экспорт
	...
	Опция = ПодготовитьОпцию("--proxy-cert", ИмяФайлаСертификата);

	Curl.ПроксиУстановитьСертификатКлиента(ИмяФайлаСертификата);
...
Процедура ТестДолжен_ПроверитьВСтрокеКомандыИспользованиеСертификатКлиентаПроксиСПаролем() Экспорт
...
Процедура ТестДолжен_ПроверитьНаличиеВКомандеЗапросаНаСжатиеОтвета() Экспорт
...
Процедура ТестДолжен_ПроверитьВСтрокеКомандыИспользованиеНезащищенногоПодключения() Экспорт
...
Процедура ТестДолжен_ПроверитьОтсутствиеПеренаправленияВРежимеЭмуляцииБраузера() Экспорт
	
	Опция = "--location";

	СтрокаКоманды = Curl
		.РазрешитьПеренаправления(Ложь)
		.ЭмуляцияБраузера()
		.СобратьКоманду("https://example.com");
		
	Ожидаем.Что(СтрокаКоманды).Не_().Содержит(Опция);
	
КонецПроцедуры

Процедура ТестДолжен_ПроверитьНаличиеПеренаправленияВРежимеЭмуляцииБраузера() Экспорт
