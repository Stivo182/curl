...
Перем РезультатВыполненияКоманды; // см. НовыйРезультатВыполненияКоманды
...
Перем СообщениеMultipart; // Массив из см. НоваяЗаписьСообщенияMultipart
Перем ОпцииПоддерживающиеМногократноеИспользование; // Соответствие
Перем ИспользоватьТихийРежим; // Булево
Перем ПоказыватьОшибкиВТихомРежиме; // Булево
Перем ПострочныйВыводОпций; // Булево
Перем ИспользоватьКраткуюФормуОпций; // Булево
Перем КраткиеФормыОпций; // Соответствие
Перем ЭтоВыполнениеЗапроса; // Булево
Перем КомандныйИнтерпретатор; // см. НовыйКомандныйИнтерпретатор
Перем ЗапроситьСжатиеОтвета; // Булево
Перем РазрешитьНезащищенноеПодключение; // Булево
...
// 
// HTTP: Если заголовок запроса Content-Type явно не передан, то MIME-тип определяется 
//       по умолчанию в зависимости от передаваемых данных:
...
// 		   - multipart/form-data: 
// 			  Если данные переданы методами: 
// 			    - см. ДобавитьФайлMultipart
// 			    - см. ДобавитьТекстMultipart
// 			    - см. ДобавитьТекстИзФайлаMultipart
...
//         - Файл
//         - Неопределено
//	МетодЗапроса - Строка - Метод запроса. Для HTTP по умолчанию POST.
// 
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция Отправить(URL, Данные = Неопределено, Знач МетодЗапроса = "") Экспорт
	МетодЗапроса = ?(ЗначениеЗаполнено(МетодЗапроса), ВРег(МетодЗапроса), ОпределитьМетодЗапроса(URL, Ложь));
	Возврат ОтправитьЗапрос(URL, Данные, МетодЗапроса);
КонецФункции
...
// 
// Для HTTP выполняется HEAD запрос.
// Для FTP и FILE получает размер файла и дата изменения.
// 
// Параметры:
//  URL - Строка - Адрес ресурса в сети Интернет
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение - HTTP заголовки:
//  	* Ключ - Строка - Имя заголовка
//  	* Значение - Строка, Массив из Строка - Значение(я) заголовка
Функция ПолучитьЗаголовки(URL) Экспорт	
	Возврат ОтправитьЗапрос(URL, , МетодЗапросаHEAD()).ЗаголовкиОтвета();	
КонецФункции

// Скачивает файл по URL и возвращает путь к файлу.
// 
// Параметры:
//  URL - Строка - Адрес ресурса в сети Интернет
// 
...
Функция СкачатьФайл(URL) Экспорт
...
// (IMAP LDAP POP3 SMTP HTTP) Устанавливает Bearer Token аутентификацию.
// 
// Параметры:
//  Токен - Строка - Токен
// 
...
Функция АутентификацияBearerToken(Токен) Экспорт
	Аутентификация = НоваяАутентификацияBearerToken();
...
// (HTTP FTP FILE) Возвращает значение заголовка ответа по имени.
// 
// Параметры:
...
// (HTTP FTP FILE) Возвращает все заголовки ответа.
// 
// Возвращаемое значение:
...
Функция РазрешитьПеренаправления(Разрешить = Истина) Экспорт	
	...
	НастройкиПеренаправления.Установлено = Истина;
	Возврат ЭтотОбъект();
КонецФункции
...
// если целевой URL использует самоподписанный сертификат.
// 
// Параметры:
//  Разрешить - Булево - Разрешить
// 
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция РазрешитьНезащищенноеПодключение(Разрешить = Истина) Экспорт
	РазрешитьНезащищенноеПодключение = Разрешить;
	Возврат ЭтотОбъект();
КонецФункции
...
// Опции выводятся построчно при выполнении сборки команды см. СобратьКоманду
// 
// Параметры:
//  ВыводитьПострочно - Булево - Выводить построчно
// 
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция ВыводитьОпцииПострочно(ВыводитьПострочно = Истина) Экспорт
	ПострочныйВыводОпций = ВыводитьПострочно;
	Возврат ЭтотОбъект();
КонецФункции
...
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция ИспользоватьКраткуюФормуОпций(Использовать = Истина) Экспорт
	ИспользоватьКраткуюФормуОпций = Использовать;
	Возврат ЭтотОбъект();	
КонецФункции

// Собирает команду curl для командной строки.
// 
// Параметры:
//  URL - Строка - Адрес ресурса в сети Интернет
//  Данные - Строка - Простой текст, путь к файлу, адрес двоичных данных во временном хранилище
...
//         - Файл
//         - Неопределено
//  МетодЗапроса - Строка - Метод запроса (GET, POST, PUT и т.д.)
//  	Для HTTP по умолчанию устанавливается POST,	когда данные есть, в остальных случаях GET.
// 
// Возвращаемое значение:
//  Строка - команда curl
Функция СобратьКоманду(URL, Данные = Неопределено, Знач МетодЗапроса = "") Экспорт
	...
	МетодЗапроса = ?(МетодЗапроса <> "",
		ВРег(МетодЗапроса),
		ОпределитьМетодЗапроса(URL, Данные = Неопределено И Не ЗначениеЗаполнено(СообщениеMultipart)));
	...
	ДобавитьОпциюМетодЗапроса(Опции, МетодЗапроса);
	...
	ДобавитьОпциюИспользованияНезащищенногоПодключения(Опции);
	ДобавитьОпциюДанныеЗапросаБезопасно(Опции, Данные, МетодЗапроса);
	...
	ДобавитьОпциюИспользованияНезащищенногоПодключенияКПрокси(Опции);
	...
	ДобавитьОпциюЗапросаСжатияОтвета(Опции);
	
	Возврат ПолучитьКоманднуюСтроку(URL, Опции);
...
Функция КодВозврата() Экспорт
	Возврат РезультатВыполненияКоманды.КодВозврата;
КонецФункции
...
Функция ТекстОшибки() Экспорт
	Возврат РезультатВыполненияКоманды.ПотокОшибок;	
КонецФункции
...
// Добавляет текст в составное (multipart) сообщение.
// 
// Параметры:
//  Имя - Строка - Имя поля
//  Текст - Строка - Текст
...
// Возвращаемое значение:
//  см. ЭтотОбъект 
Функция ДобавитьТекстMultipart(Имя, Текст, ТипMIME = "") Экспорт
	
	Запись = НоваяЗаписьСообщенияMultipart();
	Запись.ТипЗаписи = ТипТекстMultipart();
	Запись.Имя = Имя;
	Запись.Текст = Текст;
	Запись.ТипMIME = ТипMIME;
	
	СообщениеMultipart.Добавить(Запись);
	
	Возврат ЭтотОбъект();
	
КонецФункции

// Добавляет текст из файла в составное (multipart) сообщение.
// 
// Параметры:
//  Имя - Строка - Имя поля
//  Файл - Строка - Путь к файлу или адрес двоичных данных во временном хранилище 
//       - ДвоичныеДанные
//       - Файл
//  ТипMIME - Строка - MIME-тип
// 
// Возвращаемое значение:
//  см. ЭтотОбъект 
Функция ДобавитьТекстИзФайлаMultipart(Имя, Файл, ТипMIME = "") Экспорт
	
	Запись = НоваяЗаписьСообщенияMultipart();
	Запись.ТипЗаписи = ТипТекстИзФайлаMultipart();
	Запись.Имя = Имя;
	Запись.Файл = Файл;
	Запись.ТипMIME = ТипMIME;
...
// Добавляет файл в составное (multipart) сообщение.
// 
// Параметры:
//  Имя - Строка - Имя поля
//  Файл - Строка - Путь к файлу или адрес двоичных данных во временном хранилище 
...
// Возвращаемое значение:
//  см. ЭтотОбъект 
Функция ДобавитьФайлMultipart(Имя, Файл, ТипMIME = "", ИмяФайла = "") Экспорт
	
	Запись = НоваяЗаписьСообщенияMultipart();
	Запись.ТипЗаписи = ТипФайлMultipart();
	Запись.Имя = Имя;
	Запись.Файл = Файл;
	Запись.ТипMIME = ТипMIME;
...
// Очищает содержимое составного (multipart) сообщения.
// 
// Возвращаемое значение:
//  см. ЭтотОбъект  
Функция ОчиститьСообщениеMultipart() Экспорт
	СообщениеMultipart.Очистить();	
	Возврат ЭтотОбъект();
КонецФункции
...
// 
// Параметры:
//  Использовать - Булево - Использовать тихий режим
// 
// Возвращаемое значение:
//  см. ЭтотОбъект  
Функция ИспользоватьТихийРежим(Использовать = Истина) Экспорт
	ИспользоватьТихийРежим = Использовать;
	Возврат ЭтотОбъект();
КонецФункции
...
// 
// Параметры:
//  Показывать - Булево - Показывать
// 
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция ПоказыватьОшибкиВТихомРежиме(Показывать = Истина) Экспорт
	ПоказыватьОшибкиВТихомРежиме = Показывать;
	Возврат ЭтотОбъект();	
КонецФункции
...
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция УказатьФайлСохраненияРезультата(ИмяФайла) Экспорт
	ПутьКРезультату = ИмяФайла;
	Возврат ЭтотОбъект();
КонецФункции
...
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция УказатьФайлСохраненияЗаголовков(ИмяФайла) Экспорт
	ПутьКЗаголовкамОтвета = ИмяФайла;
	Возврат ЭтотОбъект();
КонецФункции

// Использует командный интерпретатор cmd.
// 
// Обработка сама определяет какой командный интерпретатор использовать в зависимости от системы. 
// Данный метод будет полезен в целях сборки команды для командной строки с помощью метода см. СобратьКоманду.
// 
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция ИспользоватьКомандныйИнтерпретаторCmd() Экспорт
	КомандныйИнтерпретатор = КомандныйИнтерпретаторCmd();
	Возврат ЭтотОбъект();
КонецФункции
...
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция ИспользоватьКомандныйИнтерпретаторBash() Экспорт
	КомандныйИнтерпретатор = КомандныйИнтерпретаторBash();
	Возврат ЭтотОбъект();
КонецФункции

// (HTTP) Запрашивает у веб-сервера ответ в сжатом виде и распаковывает его при получении.
// 
// Параметры:
//  Запрашивать - Булево - Запрашивать сжатие ответа
// 
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция ЗапроситьСжатиеОтвета(Запрашивать = Истина) Экспорт
	ЗапроситьСжатиеОтвета = Запрашивать;
	Возврат ЭтотОбъект();
КонецФункции
...
// Разрешает незащищенное подключение к прокси-серверу.
// 
// Параметры:
//  Разрешить - Булево - Разрешить
// 
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция РазрешитьНезащищенноеПодключениеКПрокси(Разрешить = Истина) Экспорт
	Прокси.РазрешитьНезащищенноеПодключение = Разрешить;
	Возврат ЭтотОбъект();
КонецФункции
...
Процедура УстановитьНачальныеНастройки()
	...
	СообщениеMultipart = Новый Массив();
	...
	ИспользоватьТихийРежим(Истина);
	ПоказыватьОшибкиВТихомРежиме(Истина);
	ВыводитьОпцииПострочно(Ложь);
	ИспользоватьКраткуюФормуОпций(Ложь);
	ЗапроситьСжатиеОтвета(Ложь);
	РазрешитьНезащищенноеПодключение(Ложь);
	УстановитьКомандныйИнтерпретаторПоУмолчанию();
...
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция ОтправитьЗапрос(URL, Данные = Неопределено, МетодЗапроса = "")

	ВключитьРежимВыполненияЗапроса();
	...
	Если Не ЗначениеЗаполнено(ПутьКРезультату) Тогда
		УказатьФайлСохраненияРезультата(НовоеИмяВременногоФайла());
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ПутьКЗаголовкамОтвета) И Не МетодЗапроса = МетодЗапросаHEAD() Тогда
		УказатьФайлСохраненияЗаголовков(НовоеИмяВременногоФайла());
	КонецЕсли;
		
	Команда = СобратьКоманду(URL, Данные, МетодЗапроса);
	
	ВыполнитьКомандуБезопасно(Команда);
	УдалитьАртефактыПослеЗапроса();
	ОчиститьСообщениеMultipart();
	ПрочитатьЗаголовкиОтвета(МетодЗапроса);
...
Процедура ВключитьРежимВыполненияЗапроса()
	ЭтоВыполнениеЗапроса = Истина;	
КонецПроцедуры

Процедура ОтключитьРежимВыполненияЗапроса()
	ЭтоВыполнениеЗапроса = Ложь;	
КонецПроцедуры

Функция АктивенРежимВыполненияЗапроса()
	Возврат ЭтоВыполнениеЗапроса;	
КонецФункции

Процедура ВыполнитьКомандуБезопасно(СтрокаКоманды)
	
	Попытка
		РезультатВыполненияКоманды = ВыполнитьКоманду(СтрокаКоманды);
		Если РезультатВыполненияКоманды.КодВозврата > 0
			И (ВыбрасыватьИсключение Или Не ЭтоОшибкаУтилиты(РезультатВыполненияКоманды.ПотокОшибок)) Тогда
			ВызватьИсключение РезультатВыполненияКоманды.ПотокОшибок;
		КонецЕсли;
	Исключение
		УдалитьАртефакты();
...
// Запускает выполнение команды на исполнение с ожиданием завершения
//
// Параметры:
//  СтрокаКоманды  - Строка - командная строка.
//
// Возвращаемое значение:
//  см. НовыйРезультатВыполненияКоманды
Функция ВыполнитьКоманду(Знач СтрокаКоманды) 
	...
	Если КомандныйИнтерпретатор.Имя = ИмяКомандногоИнтерпретатораCmd() Тогда
		ВыполнитьКомандуCmd(СтрокаКоманды, Кодировка, КодВозврата);
	ИначеЕсли КомандныйИнтерпретатор.Имя = ИмяКомандногоИнтерпретатораBash() Тогда
		ВыполнитьКомандуBash(СтрокаКоманды, Кодировка, КодВозврата);
	Иначе
		ЗапуститьПриложение(СтрокаКоманды, , Истина, КодВозврата);
	КонецЕсли;
	
	КодВозврата = ?(Не ЗначениеЗаполнено(КодВозврата), 0, КодВозврата);
...
// Новый результат выполнения команды.
// 
// Возвращаемое значение:
//  Структура:
//    * КодВозврата - Число - Код возврата
//    * ПотокОшибок - Строка - Ошибки направленные в поток stderr.
...
// (HTTP FTP FILE) Читает файл с заголовками ответа.
// 
// Параметры:
//  МетодЗапроса - Строка - Метод запроса
Процедура ПрочитатьЗаголовкиОтвета(МетодЗапроса)
			
	ЗаголовкиОтвета.Очистить();
	
	Если ЭтоHTTP(СхемаURL) Тогда
		ПутьКФайлу = ?(МетодЗапроса = МетодЗапросаHEAD(), ПутьКРезультату, ПутьКЗаголовкамОтвета);
		ПрочитатьЗаголовкиОтветаHTTP(ПутьКФайлу);
	ИначеЕсли МетодЗапроса = МетодЗапросаHEAD() Тогда
		ПрочитатьЗаголовкиHEAD(ПутьКРезультату);	
	ИначеЕсли ЭтоFTP(СхемаURL) Тогда
		ПрочитатьЗаголовкиОтветаFTP(ПутьКЗаголовкамОтвета);
	КонецЕсли;
	
	УдалитьВременныйФайл(ПутьКЗаголовкамОтвета);
		
КонецПроцедуры

Процедура ПрочитатьЗаголовкиHEAD(ПутьКФайлу)
...
Процедура ПрочитатьЗаголовкиОтветаHTTP(ПутьКФайлу)
...
Процедура ПрочитатьЗаголовкиОтветаFTP(ПутьКФайлу)
...
// Новая Bearer Token аутентификация.
// 
// Возвращаемое значение:
//  Структура - Новая аутентификация Bearer Token:
//		* Токен - Строка - Токен
Функция НоваяАутентификацияBearerToken()
	ПустаяАутентификация = НоваяАутентификацияАбстрактная();
	ПустаяАутентификация.Метод = МетодАутентификацииBearerToken();
	ПустаяАутентификация.Вставить("Токен", "");
	Возврат ПустаяАутентификация;	
КонецФункции
...
//			см. МетодАутентификацииBearerToken
...
// Оборачивает строку двойными кавычками.
// 
// Параметры:
...
// Возвращаемое значение:
//  Строка
Функция ОбернутьДвойнымиКавычками(Строка)
	Возврат СтрШаблон("%1%2%1", """", Строка);
КонецФункции
...
Функция ОбернутьКавычками(Строка)
	Возврат СтрШаблон("%1%2%1", КомандныйИнтерпретатор.СимволОборачиванияСтроки, Строка);
КонецФункции
...
// Параметры:
//  Текст - Строка - Текст
//  Символ - Строка - Экранируемый символ
// 
// Возвращаемое значение:
//  Строка
Функция ЭкранироватьСимволы(Текст, Символ = "")
	
	Результат = "";
	
	Если ЗначениеЗаполнено(Символ) Тогда
		ЭкранированныйСимвол = КомандныйИнтерпретатор.ЭкранируемыеСимволы.Получить(Символ);
		Если ЗначениеЗаполнено(ЭкранированныйСимвол) Тогда
			Результат = СтрЗаменить(Текст, Символ, ЭкранированныйСимвол);	
		Иначе
			Результат = Текст;
		КонецЕсли;		
	Иначе
		...
		Для НомерСимвола = 1 По ДлинаСтроки Цикл
			...
			ЭкранированныйСимвол = КомандныйИнтерпретатор.ЭкранируемыеСимволы.Получить(Символ);
			Если ЭкранированныйСимвол = Неопределено Тогда
				Результат = Результат + Символ;
			Иначе
				Результат = Результат + ЭкранированныйСимвол;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
// 		* РазрешитьНезащищенноеПодключение - Булево - Разрешает незащищенное соединение для HTTPS прокси
Функция НовыйПрокси()
	...
	НовыйПрокси.Вставить("РазрешитьНезащищенноеПодключение", Ложь);
	Возврат НовыйПрокси;
...
//		* Установлено - Булево - Признак установки свойства "Перенаправлять"
Функция НовыеНастройкиПеренаправления()
	...
	Параметры.Вставить("Установлено", Ложь);
	
	Возврат Параметры;
		
КонецФункции

// Новая запись составного (multipart) сообщения.
// 
// Возвращаемое значение:
//  Структура:
//      * ТипЗаписи - Строка - ТипЗаписи
//      * Имя - Строка - Имя
//      * Текст - Строка - Текст
//      * ТипMIME - Строка - MIME-тип
//      * Файл - Строка - Файл
...
//             - Неопределено
//      * ИмяФайла - Строка - Имя файла
Функция НоваяЗаписьСообщенияMultipart()
	
	Данные = Новый Структура();
	Данные.Вставить("ТипЗаписи", "");
	Данные.Вставить("Имя", "");
	Данные.Вставить("Текст", "");
	Данные.Вставить("ТипMIME", "");
	Данные.Вставить("Файл", Неопределено);
	Данные.Вставить("ИмяФайла", "");
	
	Возврат Данные;
	
КонецФункции

Функция ТипТекстMultipart()
	Возврат "Текст";
КонецФункции

Функция ТипТекстИзФайлаMultipart()
	Возврат "ТекстИзФайла";
КонецФункции

Функция ТипФайлMultipart()
	Возврат "Файл";
КонецФункции
...
// Параметры:
// 	URL - Строка - Адрес ресурса в сети Интернет
//  ЭтоМетодПолучения - Булево - Это получения данных
// 
// Возвращаемое значение:
//  Строка - Метод запроса
Функция ОпределитьМетодЗапроса(URL, ЭтоМетодПолучения)
	
	Схема = ПолучитьСхемуURL(URL);
	
	Если ЭтоHTTP(Схема) Тогда
		Если ЭтоМетодПолучения Тогда
			Возврат МетодЗапросаGET();
		Иначе
			Возврат МетодЗапросаPOST();
		КонецЕсли;		
	КонецЕсли;
...
Функция МетодЗапросаGET()
	Возврат "GET";
КонецФункции

Функция МетодЗапросаPOST()
	Возврат "POST";
КонецФункции

Функция МетодЗапросаHEAD()
	Возврат "HEAD";
КонецФункции
...
Функция НовоеИмяВременногоФайла(Расширение = "")
	...
	ВременныеФайлы.Вставить(Имя, Истина);
	Возврат Имя;
КонецФункции
...
// 
// Возвращаемое значение:
//  Булево - Это временный файл
Функция ЭтоВременныйФайл(Путь)
	Возврат ВременныеФайлы.Получить(Путь) <> Неопределено;	
КонецФункции

// Удаляет артефакты выполнения запроса.
Процедура УдалитьАртефакты()
	
	УдалитьВременныйФайл(ПутьКРезультату);	
	УдалитьВременныйФайл(ПутьКЗаголовкамОтвета);
	УдалитьВременныйФайл(ПутьКТелуЗапроса);
...
Процедура УдалитьАртефактыПослеЗапроса()
				
	УдалитьФайлЕслиСуществует(ПутьКТелуЗапроса);
	
	Если ЭтоВременныйФайл(ПутьКРезультату) Тогда
...
Процедура УдалитьВременныйФайл(Путь)
	...
	Если Не ЭтоВременныйФайл(Путь) Тогда
		Возврат;	
	КонецЕсли;
...
// Возвращаемое значение:
//  Строка
Функция ПолучитьКоманднуюСтроку(URL, Опции)
	
	Если ПострочныйВыводОпций И Не АктивенРежимВыполненияЗапроса() Тогда
		РазделительОпций = Символы.ПС + "  ";
		Суффикс = " " + КомандныйИнтерпретатор.СимволПереносаСтроки;
	Иначе
		РазделительОпций = " ";
		Суффикс = "";
	КонецЕсли;
	...
	СтрокаОпций = "";
	ВыведенныеОпции = Новый Соответствие();
	
	Если Не АктивенРежимВыполненияЗапроса() Тогда
		Для Каждого ИмяОпции Из ОпцииПоПорядку() Цикл
			Значение = Опции.Получить(ИмяОпции);
			Если Значение <> Неопределено Тогда
				ВывестиОпциюВСтроку(СтрокаОпций, ИмяОпции, Значение, РазделительОпций, Суффикс);
				ВыведенныеОпции.Вставить(ИмяОпции, Истина);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Для Каждого КлючИЗначение Из Опции Цикл		
		ИмяОпции = КлючИЗначение.Ключ; // Строка
		Значение = КлючИЗначение.Значение; // Строка, Число, Неопределено, Массив из Строка, Число
		
		Если ВыведенныеОпции.Получить(ИмяОпции) = Неопределено Тогда
			ВывестиОпциюВСтроку(СтрокаОпций, ИмяОпции, Значение, РазделительОпций, Суффикс);	
		КонецЕсли;
	КонецЦикла;
	
	Если ЗначениеЗаполнено(Суффикс) Тогда
		СтрокаОпций = Лев(СтрокаОпций, СтрДлина(СтрокаОпций) - СтрДлина(Суффикс));	
	КонецЕсли;
	
	СтрокаОпций = СокрЛП(СтрокаОпций);
	
	СтрокаКоманды = ИмяКоманды() + " " + ОбернутьКавычками(URL);
	
	Если ЗначениеЗаполнено(СтрокаОпций) Тогда
...
//  Опции - Соответствие из КлючИЗначение - Опции команды curl
//  МетодЗапроса - Строка - Метод запроса
Процедура ДобавитьОпциюМетодЗапроса(Опции, МетодЗапроса)
	
	Если Не ЗначениеЗаполнено(МетодЗапроса) Или МетодЗапроса = МетодЗапросаGET() Тогда
		Возврат;	
	КонецЕсли;
	
	Если МетодЗапроса = МетодЗапросаHEAD() Тогда
		ДобавитьОпциюВСписок(Опции, "--head");	
	Иначе	
		ДобавитьОпциюВСписок(Опции, "--request", МетодЗапроса);
	КонецЕсли;
...
Процедура ДобавитьОпциюЗаголовки(Опции)
	...
	Для Каждого КлючИЗначение Из ЗаголовкиЗапросаОбъединенные Цикл
		Заголовок = СтрШаблон("%1: %2", КлючИЗначение.Ключ, КлючИЗначение.Значение);
		ДобавитьОпциюВСписок(Опции, "--header", Заголовок);
	КонецЦикла;
...
//         - Неопределено
//	МетодЗапроса - Строка - Метод запроса
Процедура ДобавитьОпциюДанныеЗапросаБезопасно(Опции, Данные, МетодЗапроса)

	Попытка
		ДобавитьОпциюДанныеЗапроса(Опции, Данные, МетодЗапроса);
	Исключение
		УдалитьАртефакты();
...
//         - Неопределено
//	МетодЗапроса - Строка - Метод запроса
Процедура ДобавитьОпциюДанныеЗапроса(Опции, Данные, МетодЗапроса)
	
	Если Данные = Неопределено 
		И Не ЗначениеЗаполнено(СообщениеMultipart) Тогда
		Возврат;
	КонецЕсли;
		
	Если ЗначениеЗаполнено(СообщениеMultipart) Тогда
		Если Не ЭтоHTTP(СхемаURL) Тогда
			ВызватьИсключение "Передача составного (multipart) сообщения доступна только при работе с HTTP протоколом";
		ИначеЕсли МетодЗапроса <> МетодЗапросаPOST() Тогда
			ВызватьИсключение "Передача составного (multipart) сообщения доступна только в POST запросе";
		КонецЕсли;
	КонецЕсли;
	
	Если Данные <> Неопределено 
		И ЗначениеЗаполнено(СообщениеMultipart) Тогда
		ВызватьИсключение "Одновременная передача данных и составного (multipart) сообщения недоступна";
	КонецЕсли;
	
	Если Данные <> Неопределено Тогда
		ТипДанных = ТипЗнч(Данные);
		Если ТипДанных = Тип("Строка") Тогда
			ДобавитьОпциюДанныеЗапросаСтрока(Опции, Данные, МетодЗапроса);	
		ИначеЕсли ТипДанных = Тип("Структура") Или ТипДанных = Тип("Соответствие") Тогда		
			ДобавитьОпциюДанныеЗапросаСтруктура(Опции, Данные, МетодЗапроса);
		ИначеЕсли ТипДанных = Тип("ДвоичныеДанные") Тогда
			ДобавитьОпциюДанныеЗапросаДвоичныеДанные(Опции, Данные, МетодЗапроса);	
		ИначеЕсли ТипДанных = Тип("Файл") Тогда
			ДобавитьОпциюДанныеЗапросаФайл(Опции, Данные, МетодЗапроса);	
		Иначе
			ВызватьИсключение СтрШаблон("Тип %1 не поддерживается в качестве тела запроса", ТипДанных);
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(СообщениеMultipart) Тогда
		ДобавитьОпциюСодержимоеMultipart(Опции);
	КонецЕсли;
...
//  МетодЗапроса - Строка - Метод запроса
Процедура ДобавитьОпциюДанныеЗапросаСтрока(Опции, Строка, МетодЗапроса)
	...
	Если ЭтоАдресВременногоХранилища(Строка) Тогда
		...
		ДобавитьОпциюДанныеЗапросаДвоичныеДанные(Опции, ДвоичныеДанные, МетодЗапроса);	
	ИначеЕсли ФайлСуществует(Строка) Тогда
		Файл = Новый Файл(Строка);
		ДобавитьОпциюДанныеЗапросаФайл(Опции, Файл, МетодЗапроса);
	ИначеЕсли ЭтоHTTP(СхемаURL) Тогда
		УдалитьЗапрещенныеСимволыОтправляемыхДанных(Строка);
		ДобавитьОпциюВСписок(Опции, "--data-raw", Строка);
	Иначе
		ВызватьИсключение СтрШаблон("Передача данных с типом 'Строка' не реализована для протокола %1", СхемаURL);
	КонецЕсли;
...
//            - Соответствие из КлючИЗначение
//  МетодЗапроса - Строка - Метод запроса
Процедура ДобавитьОпциюДанныеЗапросаСтруктура(Опции, Структура, МетодЗапроса)
	
	Тип = ТипЗнч(Структура);
	
	Проверить(
		Тип = Тип("Структура") Или Тип = Тип("Соответствие"), 
		"Ожидалось получение структуры");
	
	Если ЭтоHTTP(СхемаURL) Тогда
		ТелоЗапроса = СтруктураВТелоЗапроса(Структура);	
		ДобавитьОпциюДанныеЗапросаСтрока(Опции, ТелоЗапроса, МетодЗапроса);		
	Иначе
		ВызватьИсключение СтрШаблон("Передача данных с типом '%1' не реализована", Тип);
	КонецЕсли;
...
//  МетодЗапроса - Строка - Метод запроса
Процедура ДобавитьОпциюДанныеЗапросаДвоичныеДанные(Опции, ДвоичныеДанные, МетодЗапроса)
...
//  Файл - Файл - Файл
//  МетодЗапроса - Строка - Метод запроса
Процедура ДобавитьОпциюДанныеЗапросаФайл(Опции, Файл, МетодЗапроса)
	
	Проверить(ТипЗнч(Файл) = Тип("Файл"), "Ожидалось получение файла");
	
	Если ЭтоHTTP(СхемаURL) И МетодЗапроса = МетодЗапросаPOST() Тогда
...
// Добавляет содержимое составного (multipart) сообщения в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции
Процедура ДобавитьОпциюСодержимоеMultipart(Опции)
	
	Для Каждого Запись Из СообщениеMultipart Цикл
		Если Запись.ТипЗаписи = ТипТекстMultipart() Тогда
			ДобавитьОпциюТекстMultipart(Опции, Запись);
		Иначе
			ДобавитьОпциюФайлMultipart(Опции, Запись);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

// Добавляет текст составного (multipart) сообщения в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции
//  Запись - см. НоваяЗаписьСообщенияMultipart
Процедура ДобавитьОпциюТекстMultipart(Опции, Запись)
	
	ЗакодированнаяДвойнаяКавычка = "%22";
	
	ИмяСвойства = СтрЗаменить(Запись.Имя, """", ЗакодированнаяДвойнаяКавычка);
	ЗначениеСвойства = СтрЗаменить(Запись.Текст, """", ЗакодированнаяДвойнаяКавычка);
	
	ЗначениеОпции = СтрШаблон("%1=%2", 
		ИмяСвойства, 
		ОбернутьДвойнымиКавычками(ЗначениеСвойства));
		
	Если ЗначениеЗаполнено(Запись.ТипMIME) Тогда
		ЗначениеОпции = ЗначениеОпции + ";type=" + Запись.ТипMIME;
	КонецЕсли;
	
	ДобавитьОпциюВСписок(Опции, "--form", ЗначениеОпции);
	
КонецПроцедуры

// Добавляет файл составного (multipart) сообщения в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции
//  Запись - см. НоваяЗаписьСообщенияMultipart
Процедура ДобавитьОпциюФайлMultipart(Опции, Запись)

	ЗакодированнаяДвойнаяКавычка = "%22";
	
	ТипЗначения = ТипЗнч(Запись.Файл);
	
	Файл = Неопределено;
	Если ТипЗначения = Тип("ДвоичныеДанные") Тогда
		ПутьКФайлу = НовоеИмяВременногоФайла();	
		Запись.Файл.Записать(ПутьКФайлу);
		Файл = Новый Файл(ПутьКФайлу);
	ИначеЕсли ТипЗначения = Тип("Строка") Тогда
		ПутьКФайлу = "";
		Если ЭтоАдресВременногоХранилища(Запись.Файл) Тогда
			ПутьКФайлу = НовоеИмяВременногоФайла();
			ДвоичныеДанные = ПолучитьИзВременногоХранилища(Запись.Файл); // ДвоичныеДанные
			ДвоичныеДанные.Записать(ПутьКФайлу);
		Иначе
			ПутьКФайлу = Запись.Файл;
		КонецЕсли;	
		Файл = Новый Файл(ПутьКФайлу);
	ИначеЕсли ТипЗначения <> Тип("Файл") Тогда
		ВызватьИсключение СтрШаблон("Файл с типом значения %1 не поддерживается", ТипЗначения);
	КонецЕсли;
				
	ИмяСвойства = СтрЗаменить(Запись.Имя, """", ЗакодированнаяДвойнаяКавычка);
	
	Если Запись.ТипЗаписи = ТипФайлMultipart() Тогда
	
		ЗначениеОпции = СтрШаблон("%1=@%2", 
			ИмяСвойства, 
			ОбернутьДвойнымиКавычками(Файл.ПолноеИмя));
			
		Если ЗначениеЗаполнено(Запись.ИмяФайла) Тогда
			ИмяФайла = СтрЗаменить(Запись.ИмяФайла, """", ЗакодированнаяДвойнаяКавычка);
			ЗначениеОпции = ЗначениеОпции + ";filename=" + ИмяФайла;
		КонецЕсли;
...
Процедура ДобавитьОпциюФайлДампаЗаголовков(Опции)
	...
	Если Не (ЭтоHTTP(СхемаURL) Или ЭтоFTP(СхемаURL)) Тогда
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПутьКЗаголовкамОтвета) Тогда
...
Процедура ДобавитьОпциюАутентификации(Опции)
	...
	ИначеЕсли Аутентификация.Метод = МетодАутентификацииBearerToken() Тогда
		ДобавитьОпциюАутентификацииBearerToken(Опции);
	Иначе
...
// (IMAP LDAP POP3 SMTP HTTP) Добавляет Bearer Token аутентификацию в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции команды curl
Процедура ДобавитьОпциюАутентификацииBearerToken(Опции)
	
	АутентификацияBearerToken = Аутентификация; // см. НоваяАутентификацияBearerToken
	
	ДобавитьОпциюВСписок(Опции, "--oauth2-bearer", АутентификацияBearerToken.Токен);
...
// Добавляет опцию использования незащещенного подключения в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции команды curl
Процедура ДобавитьОпциюИспользованияНезащищенногоПодключения(Опции)
	Если РазрешитьНезащищенноеПодключение Тогда
		ДобавитьОпциюВСписок(Опции, "--insecure");
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьОпциюПутьКВыходномуФайлу(Опции)
	Если ЗначениеЗаполнено(ПутьКРезультату) Тогда
...
Процедура ДобавитьОпциюСертификатаКлиента(Опции)
	...
	// --cert
	Если ЗначениеЗаполнено(СертификатКлиента.Пароль) Тогда
		Значение = СтрШаблон("%1:%2", СертификатКлиента.ИмяФайлаСертификата, СертификатКлиента.Пароль);
	Иначе
		Значение = СертификатКлиента.ИмяФайлаСертификата;
	КонецЕсли;
...
// Добавляет опцию использования незащещенного подключения к прокси-серверу в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции команды curl
Процедура ДобавитьОпциюИспользованияНезащищенногоПодключенияКПрокси(Опции)
	Если Прокси.РазрешитьНезащищенноеПодключение Тогда
		ДобавитьОпциюВСписок(Опции, "--proxy-insecure");
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьОпциюСертификатаКлиентаПрокси(Опции)
	...
	Если ЗначениеЗаполнено(Прокси.СертификатКлиента.Пароль) Тогда
		Значение = СтрШаблон("%1:%2", Прокси.СертификатКлиента.ИмяФайлаСертификата, Прокси.СертификатКлиента.Пароль);
	Иначе
		Значение = Прокси.СертификатКлиента.ИмяФайлаСертификата;
	КонецЕсли;
...
Процедура ДобавитьОпциюТихийРежим(Опции)
	Если ИспользоватьТихийРежим Тогда
...
Процедура ДобавитьОпциюПоказыватьОшибки(Опции)
	Если ПоказыватьОшибкиВТихомРежиме Тогда
...
Процедура ДобавитьОпциюВСписок(Опции, Знач Имя, Знач Значение = Неопределено)
	
	Если Значение = Неопределено Тогда
		Значение = Истина;
	КонецЕсли;
	
	Если ЭтоОпцияМногократногоИспользования(Имя) Тогда
		ТекущееЗначение = Опции.Получить(Имя); // Строка, Число, Неопределено, Массив из Строка, Число
		...
		Иначе
			МассивЗначений = Новый Массив; // Массив из Строка, Число
			МассивЗначений.Добавить(ТекущееЗначение);
			МассивЗначений.Добавить(Значение);
			Опции.Вставить(Имя, МассивЗначений);
		КонецЕсли;
	Иначе
		Опции.Вставить(Имя, Значение);
	КонецЕсли;
...
// Добавляет опцию использования сжатия в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции команды curl
Процедура ДобавитьОпциюЗапросаСжатияОтвета(Опции)
	
	Если ЗапроситьСжатиеОтвета Тогда
		ДобавитьОпциюВСписок(Опции, "--compressed");
	КонецЕсли;
	
КонецПроцедуры

// Вывести опцию в строку.
// 
// Параметры:
//  СтрокаОпций - Строка - Строка опций
//  ИмяОпции - Строка - Имя опции
//  Значение - Строка, Число, Неопределено, Массив из Строка, Число - Значение
//  РазделительОпций - Строка - Разделитель опций
//  Суффикс - Строка - Суффикс
Процедура ВывестиОпциюВСтроку(СтрокаОпций, Знач ИмяОпции, Значение, РазделительОпций, Суффикс)
		
	Если ИспользоватьКраткуюФормуОпций Тогда
		ИмяОпции = КраткаяФормаОпции(ИмяОпции);	
	КонецЕсли;
	...
	Для Каждого ТекущееЗначение Из МассивЗначений Цикл
		ВыводитьЗначение = ЗначениеЗаполнено(ТекущееЗначение) И ТипЗнч(ТекущееЗначение) <> Тип("Булево");
		
		СтрокаОпций = СтрокаОпций 
			+ РазделительОпций
			+ ИмяОпции
			+ ?(ВыводитьЗначение, " " + ПодготовитьЗначениеПараметраКоманды(ТекущееЗначение), "")
			+ Суффикс;
	КонецЦикла;
	
КонецПроцедуры

Функция ЭтоОпцияМногократногоИспользования(Опция)
	Возврат ОпцииПоддерживающиеМногократноеИспользование().Получить(Опция) <> Неопределено;
КонецФункции

Функция ОпцииПоддерживающиеМногократноеИспользование()
	
	Если Не ОпцииПоддерживающиеМногократноеИспользование = Неопределено Тогда
		Возврат ОпцииПоддерживающиеМногократноеИспользование;
	КонецЕсли;
	
	ПереченьОпций = "-F,--form,-H,--header";
	
	ОпцииПоддерживающиеМногократноеИспользование = Новый Соответствие();
	
	Для Каждого Опция Из СтрРазделить(ПереченьОпций, ",|") Цикл
		ОпцииПоддерживающиеМногократноеИспользование.Вставить(Опция, Истина);	
	КонецЦикла;
...
// 
// Возвращаемое значение:
//  Строка - Краткая форма опции
Функция КраткаяФормаОпции(Опция)
	КраткаяФорма = КраткиеФормыОпций().Получить(Опция);
	Если Не КраткаяФорма = Неопределено Тогда
		Возврат КраткаяФорма;
	КонецЕсли;
	Возврат Опция;
КонецФункции

Функция КраткиеФормыОпций()
	
	Если Не КраткиеФормыОпций = Неопределено Тогда
		Возврат КраткиеФормыОпций;	
	КонецЕсли;
	
	КраткиеФормыОпций = Новый Соответствие;
	КраткиеФормыОпций.Вставить("--request", "-X");
	КраткиеФормыОпций.Вставить("--header", "-H");
	КраткиеФормыОпций.Вставить("--upload-file", "-T");
	КраткиеФормыОпций.Вставить("--form", "-F");
	КраткиеФормыОпций.Вставить("--dump-header", "-D");
	КраткиеФормыОпций.Вставить("--user", "-u");
	КраткиеФормыОпций.Вставить("--location", "-L");
	КраткиеФормыОпций.Вставить("--output", "-o");
	КраткиеФормыОпций.Вставить("--cert", "-E");
	КраткиеФормыОпций.Вставить("--proxy", "-x");
	КраткиеФормыОпций.Вставить("--proxy-user", "-U");
	КраткиеФормыОпций.Вставить("--silent", "-s");
	КраткиеФормыОпций.Вставить("--show-error", "-S");
	КраткиеФормыОпций.Вставить("--max-time", "-m");
	КраткиеФормыОпций.Вставить("--speed-limit", "-Y");
	КраткиеФормыОпций.Вставить("--speed-time", "-y");
	КраткиеФормыОпций.Вставить("--head", "-I");	
	КраткиеФормыОпций.Вставить("--insecure", "-k");	
	Возврат КраткиеФормыОпций;
...
Функция ОпцииПоПорядку()
	...
	МассивОпций = Новый Массив(); // Массив из Строка
	МассивОпций.Добавить("--request");
	МассивОпций.Добавить("--head");
	МассивОпций.Добавить("--data-raw");
	МассивОпций.Добавить("--data-binary");
	МассивОпций.Добавить("--form");
	МассивОпций.Добавить("--upload-file");
	МассивОпций.Добавить("--header");
	
	МассивОпций.Добавить("--user");
	МассивОпций.Добавить("--digest");
	МассивОпций.Добавить("--aws-sigv4");
	МассивОпций.Добавить("--oauth2-bearer");	
	
	МассивОпций.Добавить("--proxy");
	МассивОпций.Добавить("--proxy-user");
	МассивОпций.Добавить("--proxy-digest");
	МассивОпций.Добавить("--proxy-cert");
	МассивОпций.Добавить("--proxy-cert-type");
	МассивОпций.Добавить("--proxy-key");
	МассивОпций.Добавить("--proxy-key-type");
	МассивОпций.Добавить("--proxy-pass");
	МассивОпций.Добавить("--proxy-ca-native");
	МассивОпций.Добавить("--proxy-cacert");
	МассивОпций.Добавить("--proxy-insecure");
	
	МассивОпций.Добавить("--cert");
	МассивОпций.Добавить("--cert-type");
	МассивОпций.Добавить("--key");
	МассивОпций.Добавить("--key-type");
	МассивОпций.Добавить("--pass");
	МассивОпций.Добавить("--ca-native");
	МассивОпций.Добавить("--cacert");
	
	МассивОпций.Добавить("--output");
	МассивОпций.Добавить("--dump-header");

	МассивОпций.Добавить("--ciphers");
	МассивОпций.Добавить("--location");
	МассивОпций.Добавить("--location-trusted");
	МассивОпций.Добавить("--max-redirs");
			
	МассивОпций.Добавить("--insecure");
	МассивОпций.Добавить("--compressed");
	МассивОпций.Добавить("--connect-timeout");
	МассивОпций.Добавить("--retry");
	МассивОпций.Добавить("--retry-delay");
	МассивОпций.Добавить("--retry-max-time");
	МассивОпций.Добавить("--limit-rate");
	МассивОпций.Добавить("--max-time");
	МассивОпций.Добавить("--speed-limit");
	МассивОпций.Добавить("--speed-time");

	МассивОпций.Добавить("--silent");
	МассивОпций.Добавить("--show-error");
...
Процедура ЭмуляцияChrome(Опции)
	...
	Если Не НастройкиПеренаправления.Установлено Тогда
...
#Область КомандныйИнтерпретатор

// Устанавливает командный интерпретатор по умолчанию.
// 
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция УстановитьКомандныйИнтерпретаторПоУмолчанию()
...
// Подготавливает значение параметра команды.
// 
// Параметры:
...
// Возвращаемое значение:
//  Строка
Функция ПодготовитьЗначениеПараметраКоманды(Значение)
	Если КомандныйИнтерпретатор.Имя = ИмяКомандногоИнтерпретатораCmd() Тогда
		Возврат ПодготовитьЗначениеПараметраCmd(Значение); 		
	ИначеЕсли КомандныйИнтерпретатор.Имя = ИмяКомандногоИнтерпретатораBash() Тогда
		Возврат ПодготовитьЗначениеПараметраBash(Значение);
	Иначе
		ВызватьИсключение "Командный интерпретатор не определен";
	КонецЕсли;	
КонецФункции

// Новые командный интерпретатор.
// 
// Возвращаемое значение:
//  Структура:
//  	* Имя - Строка - Имя интерпретатора
//		* ЭкранируемыеСимволы - Соответствие из КлючИЗначение - Экранируемые символы:
//			** Ключ - Строка - Экранируемый символ
//			** Значение - Строка - Символ с экранированием
//		* СимволОборачиванияСтроки - Строка - Символ оборачивания строки
//		* СимволПереносаСтроки - Строка - Символ перенос строки
Функция НовыйКомандныйИнтерпретатор()
	Свойства = Новый Структура();
	Свойства.Вставить("Имя", "");
	Свойства.Вставить("ЭкранируемыеСимволы", Новый Соответствие());
	Свойства.Вставить("СимволОборачиванияСтроки", "");
	Свойства.Вставить("СимволПереносаСтроки", "");
	Возврат Свойства;
КонецФункции
...
// 
// Возвращаемое значение:
//  см. НовыйКомандныйИнтерпретатор
Функция КомандныйИнтерпретаторCmd()
	Свойства = НовыйКомандныйИнтерпретатор();
	Свойства.Имя = ИмяКомандногоИнтерпретатораCmd();
	Свойства.ЭкранируемыеСимволы = ЭкранируемыеСимволыCmd();
	Свойства.СимволОборачиванияСтроки = СимволОборачиванияСтрокиCmd();
	Свойства.СимволПереносаСтроки = "^";
	Возврат Свойства;
КонецФункции
...
//  Кодировка - КодировкаТекста - Возвращаемая кодировка
//  КодВозврата - Число - Возвращаемый код возврата
Процедура ВыполнитьКомандуCmd(СтрокаКоманды, Кодировка, КодВозврата)
...
// Подготавливает значение параметра (cmd).
// 
// Параметры:
//  Значение - Строка, Число - Значение параметра
// 
// Возвращаемое значение:
//  Строка
Функция ПодготовитьЗначениеПараметраCmd(Значение)
	ТипЗначения = ТипЗнч(Значение);
	Если ТипЗначения = Тип("Число") Тогда
		Возврат Формат(Значение, "ЧРД=.; ЧГ=;");
	ИначеЕсли ТипЗначения = Тип("Строка") Тогда
		Если АктивенРежимВыполненияЗапроса() Тогда
			Результат = ЭкранироватьСимволы(Значение, СимволОборачиванияСтрокиCmd());
			
			// Обработка обратной косой черты в конце строки
			Позиция = СтрДлина(Результат);
			НакопленнаяОбратнаяКосаяЧерта = "";
			Пока Позиция > 0 И Сред(Результат, Позиция, 1) = "\" Цикл
				НакопленнаяОбратнаяКосаяЧерта = НакопленнаяОбратнаяКосаяЧерта + "\";	
				Позиция = Позиция - 1;
			КонецЦикла;
			
			Результат = Результат + НакопленнаяОбратнаяКосаяЧерта;
		Иначе
			Результат = ЭкранироватьСимволы(Значение);
		КонецЕсли;	
		
		Результат = ОбернутьКавычками(Результат);
		Возврат Результат;
	Иначе
		ВызватьИсключение СтрШаблон("Значения параметров команды с типом '%1' не поддерживаются", ТипЗначения);
	КонецЕсли;
КонецФункции

Функция ЭкранируемыеСимволыCmd()
	ЭкранируемыеСимволы = Новый Соответствие();
	ЭкранируемыеСимволы.Вставить("%", "%%");	
	ЭкранируемыеСимволы.Вставить("""", """""");
	Возврат ЭкранируемыеСимволы;
КонецФункции
...
// Возвращаемое значение:
//  см. НовыйКомандныйИнтерпретатор
Функция КомандныйИнтерпретаторBash()
	Свойства = НовыйКомандныйИнтерпретатор();
	Свойства.Имя = ИмяКомандногоИнтерпретатораBash();
	Свойства.ЭкранируемыеСимволы = ЭкранируемыеСимволыBash();
	Свойства.СимволОборачиванияСтроки = СимволОборачиванияСтрокиBash();
	Свойства.СимволПереносаСтроки = "\";
	Возврат Свойства;	
КонецФункции
...
//  Кодировка - КодировкаТекста - Возвращаемая кодировка
//  КодВозврата - Число - Возвращаемый код возврата
Процедура ВыполнитьКомандуBash(СтрокаКоманды, Кодировка, КодВозврата)

	Кодировка = КодировкаТекста.UTF8;
	
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ТекстовыйДокумент.ДобавитьСтроку("#!/bin/sh");
	ТекстовыйДокумент.ДобавитьСтроку(СтрокаКоманды);
	
	ПутьКСкрипту = НовоеИмяВременногоФайла("sh");
	ТекстовыйДокумент.Записать(ПутьКСкрипту, Кодировка, Символы.ПС);
	ЗапуститьПриложение("bash " + ПутьКСкрипту, , Истина, КодВозврата);
	УдалитьВременныйФайл(ПутьКСкрипту);

КонецПроцедуры

// Подготавливает значение параметра (bash).
// 
// Параметры:
//  Значение - Строка, Число - Значение параметра
// 
// Возвращаемое значение:
//  Строка
Функция ПодготовитьЗначениеПараметраBash(Значение)
	ТипЗначения = ТипЗнч(Значение);
	Если ТипЗначения = Тип("Число") Тогда
		Возврат Формат(Значение, "ЧРД=.; ЧГ=;");
	ИначеЕсли ТипЗначения = Тип("Строка") Тогда
		Если СтрНайти(Значение, СимволОборачиванияСтрокиBash()) Тогда
			Префикс = "$";
			Результат = ЭкранироватьСимволы(Значение);
		Иначе
			Префикс = "";
		...
		Результат = Префикс + ОбернутьКавычками(Результат);
		Возврат Результат;
	Иначе
		ВызватьИсключение СтрШаблон("Значения параметров команды с типом '%1' не поддерживаются", ТипЗначения);
	КонецЕсли;	
...
Функция ЭкранируемыеСимволыBash()
	ЭкранируемыеСимволы = Новый Соответствие();
	ЭкранируемыеСимволы.Вставить("\", "\\");
	ЭкранируемыеСимволы.Вставить("!", "\041");	
	ЭкранируемыеСимволы.Вставить("'", "\'");
	Возврат ЭкранируемыеСимволы;
КонецФункции
