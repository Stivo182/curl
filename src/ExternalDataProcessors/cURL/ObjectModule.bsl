...
Функция ДобавитьШифры(Шифры) Экспорт
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(НаборШифров, СтрРазделить(Шифры, ","), Истина);
	Возврат ЭтотОбъект;
КонецФункции
...
// Возвращаемое значение:
//  ОбработкаОбъект.cURL
Функция УстановитьИсполняемыйФайл(Путь) Экспорт
...
// Возвращаемое значение:
//  ОбработкаОбъект.cURL
Функция ОтправитьЗапрос(URL, Метод, Тело = "")
...
Процедура УстановитьНачальныеНастройки()
	...
	УстановитьИсполняемыйФайл(ОбщегоНазначенияИнвесторПовтИсп.ПутьКCURL());
...
Функция НоваяКоманда()
	...
	Если Не ЗначениеЗаполнено(ПутьКИсполняемомуФайлу) Тогда
		ВызватьИсключение "Не указан путь к исполняемому файлу cURL";
	КонецЕсли;
		
	Возврат СтрШаблон("""%1""", ПутьКИсполняемомуФайлу);
...
Процедура ДобавитьТелоЗапросаВПараметрыКоманды(Команда, Тело)
	...
	ДобавитьПараметрКоманды(Команда, "--data-raw", ОбернутьКавычками(Тело));	
