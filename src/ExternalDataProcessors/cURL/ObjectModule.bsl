...
// Оборачивает строку экранированными кавычками.
// 
// Параметры:
//  Строка - Строка - Строка
//  СимволЭкранирования - Строка - Символ экранирования
// 
// Возвращаемое значение:
//  Строка
Функция ОбернутьЭкранированнымиКавычками(Строка, СимволЭкранирования = "\")
	Возврат СтрШаблон("%1""%2%1""", СимволЭкранирования, Строка);
КонецФункции
...
Процедура УдалитьАртефакты()
	...
	УдалитьВременныйФайл(ПутьКЗаголовкамОтвета);
	УдалитьВременныйФайл(ПутьКЗаголовкамЗапроса);
...
Процедура ДобавитьОпциюЗаписьMultipart(Опции, Элемент)
	...
	ЗначениеОпции = СтрШаблон("%1=%2", 
		ИмяСвойства, 
		ОбернутьЭкранированнымиКавычками(ЗначениеСвойства));
...
Процедура ДобавитьОпциюФайлMultipart(Опции, Элемент)
	...
	ЗначениеОпции = СтрШаблон("%1=@%2", 
		ИмяСвойства, 
		ОбернутьЭкранированнымиКавычками(Файл.ПолноеИмя));
	...
	Для Каждого КлючИЗначение Из Опции Цикл
		...
		Для Каждого ТекущееЗначение Из МассивЗначений Цикл
			...
			Если ТипЗначения = Тип("Строка") Тогда
				СтрокаОпций = СтрокаОпций + " " + ОбернутьКавычками(ТекущееЗначение);	
			ИначеЕсли ТипЗначения = Тип("Число") Тогда
				СтрокаОпций = СтрокаОпций + " " + Формат(ТекущееЗначение, "ЧРД=.; ЧГ=;");
			Иначе
