...
Перем ДанныеMultipart; // Массив из см. НоваяСущностьMultipart
...
// 		   - multipart/form-data: 
// 			  Если данные переданы методами см. ДобавитьЗаписьMultipart и см. ДобавитьФайлMultipart
...
Функция СобратьКоманду(URL, Данные = Неопределено, Знач МетодЗапроса = "") Экспорт
	...
	МетодЗапроса = ?(МетодЗапроса <> "",
		ВРег(МетодЗапроса),
		ОпределитьМетодЗапроса(URL, Данные = Неопределено И Не ЗначениеЗаполнено(ДанныеMultipart)));
...
// Добавляет запись в multipart сообщение.
// 
// Параметры:
//  Имя - Строка - Имя поля
//  Значение - Строка - Значение
...
// Возвращаемое значение:
//  см. ЭтотОбъект 
Функция ДобавитьЗаписьMultipart(Имя, Значение, ТипMIME = "") Экспорт
	
	Элемент = НоваяСущностьMultipart();
	Элемент.Имя = Имя;
	Элемент.Значение = Значение;
	Элемент.ТипMIME = ТипMIME;
	
	ДанныеMultipart.Добавить(Элемент);
	
	Возврат ЭтотОбъект();
	
КонецФункции

// Добавляет файл в multipart сообщение.
// 
// Параметры:
//  Имя - Строка - Имя поля
...
// Очищает содержимое multipart.
// 
// Возвращаемое значение:
//  см. ЭтотОбъект  
Функция ОчиститьСодержимоеMultipart() Экспорт
	ДанныеMultipart.Очистить();	
	Возврат ЭтотОбъект();
КонецФункции
...
// Новая сущность multipart сообщения.
// 
// Возвращаемое значение:
//  Структура:
//      * Имя - Строка - Имя
...
//             - Неопределено
//      * ИмяФайла - Строка - Имя файла
Функция НоваяСущностьMultipart()
	
	Данные = Новый Структура();
	Данные.Вставить("Имя", "");
	Данные.Вставить("Значение", "");
...
Процедура ДобавитьОпциюДанныеЗапроса(Опции, Данные, МетодЗапроса)
	...
	Если Данные = Неопределено 
		И Не ЗначениеЗаполнено(ДанныеMultipart) Тогда
		Возврат;
	КонецЕсли;
		
	Если ЗначениеЗаполнено(ДанныеMultipart) Тогда
		Если Не ЭтоHTTP(СхемаURL) Тогда
			ВызватьИсключение "Передача сообщения multipart доступна только при работе с HTTP протоколом";
		ИначеЕсли МетодЗапроса <> МетодЗапросаPOST() Тогда
			ВызватьИсключение "Передача сообщения multipart доступна только в POST запросе";
		КонецЕсли;
	КонецЕсли;
	
	Если Данные <> Неопределено 
		И ЗначениеЗаполнено(ДанныеMultipart) Тогда
		ВызватьИсключение "Одновременная передача дискретных и multipart данных недоступна";
	КонецЕсли;
	...
	Если Данные <> Неопределено Тогда
		...
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(ДанныеMultipart) Тогда
...
// Добавляет содержимое multipart в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции
Процедура ДобавитьОпциюСодержимоеMultipart(Опции)
	
	Для Каждого Элемент Из ДанныеMultipart Цикл
		Если Элемент.Файл <> Неопределено Тогда
			ДобавитьОпциюФайлMultipart(Опции, Элемент);
		Иначе
			ДобавитьОпциюЗаписьMultipart(Опции, Элемент);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

// Добавляет запись multipart сообщения в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции
//  Элемент - см. НоваяСущностьMultipart
Процедура ДобавитьОпциюЗаписьMultipart(Опции, Элемент)
	
	ЗакодированнаяДвойнаяКавычка = "%22";
	
	ИмяСвойства = СтрЗаменить(Элемент.Имя, """", ЗакодированнаяДвойнаяКавычка);
	ЗначениеСвойства = СтрЗаменить(Элемент.Значение, """", ЗакодированнаяДвойнаяКавычка);
	
	ЗначениеОпции = СтрШаблон("%1=%2", 
		ИмяСвойства, 
		ОбернутьДвойнымиКавычками(ЗначениеСвойства));
		
	Если ЗначениеЗаполнено(Элемент.ТипMIME) Тогда
		ЗначениеОпции = ЗначениеОпции + ";type=" + Элемент.ТипMIME;
	КонецЕсли;
...
// Добавляет файл multipart сообщения в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции
//  Элемент - см. НоваяСущностьMultipart
Процедура ДобавитьОпциюФайлMultipart(Опции, Элемент)

	ЗакодированнаяДвойнаяКавычка = "%22";
	
	ТипЗначения = ТипЗнч(Элемент.Файл);
	
	Файл = Неопределено;
	Если ТипЗначения = Тип("ДвоичныеДанные") Тогда
		ПутьКФайлу = НовоеИмяВременногоФайла();	
		Элемент.Файл.Записать(ПутьКФайлу);
		Файл = Новый Файл(ПутьКФайлу);
	ИначеЕсли ТипЗначения = Тип("Строка") Тогда
		ПутьКФайлу = "";
		Если ЭтоАдресВременногоХранилища(Элемент.Файл) Тогда
			ПутьКФайлу = НовоеИмяВременногоФайла();
			ДвоичныеДанные = ПолучитьИзВременногоХранилища(Элемент.Файл); // ДвоичныеДанные
			ДвоичныеДанные.Записать(ПутьКФайлу);
		Иначе
			ПутьКФайлу = Элемент.Файл;
		КонецЕсли;	
		Файл = Новый Файл(ПутьКФайлу);
	...
	Если ЗначениеЗаполнено(Элемент.ИмяФайла) Тогда
		ИмяФайла = СтрЗаменить(Элемент.ИмяФайла, """", ЗакодированнаяДвойнаяКавычка);
