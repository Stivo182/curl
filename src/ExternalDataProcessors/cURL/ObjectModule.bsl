...
Перем ИспользоватьСжатие; // Булево
...
Функция СобратьКоманду(URL, Данные = Неопределено, Знач МетодЗапроса = "") Экспорт
	...
	ДобавитьОпциюИспользованияСжатия(Опции);
...
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция ИспользоватьСжатие(Использовать = Истина) Экспорт
	ИспользоватьСжатие = Использовать;
	Возврат ЭтотОбъект();
КонецФункции
...
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции команды curl
Процедура ДобавитьОпциюИспользованияСжатия(Опции)
