...
// 
// HTTP: Передача заголовка Content-Type по умолчнию в зависимости от типа передаваемых данных:
// 		- application/octet-stream:
// 			- ДвоичныеДанные
// 			- Файл
// 			- Строка - содержащий путь к существующему файлу на диске
// 			- Строка - содержащий адрес двоичных данных во временном хранилище
// 		- application/x-www-form-urlencoded: для всех остальных
// 
// Параметры:
//  URL - Строка - Адрес ресурса в сети Интернет
...
Функция СобратьКоманду(URL, Данные = Неопределено, Знач Метод = "") Экспорт
	...
	Метод = ?(Метод <> "", ВРег(Метод), ОпределитьМетодЗапроса(URL, Данные = Неопределено));
