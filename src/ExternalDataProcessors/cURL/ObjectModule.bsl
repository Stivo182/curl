...
Перем ОтправлятьДанныеАутентификацииПриПеренаправленииНаДругойХост; // Булево
...
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция РазрешатьПеренаправления(ПеренаправлятьЗапрос = Истина, ОтправлятьДанныеАутентификацииНаДругойХост = Ложь) Экспорт
	ПеренаправлятьЗапросПоНовомуURI = ПеренаправлятьЗапрос;
	ОтправлятьДанныеАутентификацииПриПеренаправленииНаДругойХост = ОтправлятьДанныеАутентификацииНаДругойХост;
	Возврат ЭтотОбъект();
КонецФункции
...
// 
// Невозможность выполнения запроса за заданое время приводит к
...
// 
// Невозможность подключения в течение заданого времени приводит к
// завершению работы curl с кодом возврата (28).
// 
// Параметры:
//  Секунд - Число - Секунд
...
// 
// Имеет смысл вызывать совместно с методом см. УстановитьКоличествоПопыток.
// 
// Параметры:
//  Секунд - Число - Секунд
// 
...
Процедура ДобавитьОпциюЗаголовки(Опции)
	...
	Если ЗаголовкиЗапроса = Неопределено 
		Или ЗаголовкиЗапроса.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьОпциюПеренаправлениеЗапроса(Опции)
	...
	Если ОтправлятьДанныеАутентификацииПриПеренаправленииНаДругойХост Тогда
...
// Добавляет таймаут в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции команды curl
...
// Добавляет таймаут соединения в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции команды curl
...
// Добавляет количество повторных попыток в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции команды curl
...
// Добавляет задержку повторной попытки в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции команды curl
...
// Добавляет таймаут повторных попыток выполнения запроса в список опций команды.
// 
// Параметры:
//  Опции - Соответствие из КлючИЗначение - Опции команды curl
