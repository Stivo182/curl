...
Функция Получить(URL) Экспорт
	Метод = ОпределитьМетодЗапроса(URL, Истина);
	Возврат ОтправитьЗапрос(URL, , Метод);
КонецФункции
...
//         - Файл
//         - Неопределено
//	Метод - Строка - Метод запроса. Для HTTP по умолчанию POST.
// 
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция Отправить(URL, Данные = Неопределено, Знач Метод = "") Экспорт
	Метод = ?(ЗначениеЗаполнено(Метод), Метод, ОпределитьМетодЗапроса(URL, Ложь));
	Возврат ОтправитьЗапрос(URL, Данные, Метод);
КонецФункции
...
Функция СобратьКоманду(URL, Данные = Неопределено, Знач Метод = "") Экспорт
...
// Возвращаемое значение:
//  см. ЭтотОбъект
Функция ОтправитьЗапрос(URL, Данные = Неопределено, Метод = "")
	...
	Если Не ЗначениеЗаполнено(ПутьКЗаголовкамОтвета) И Не Метод = МетодЗапросаHEAD() Тогда
		УказатьФайлСохраненияЗаголовков(НовоеИмяВременногоФайла());
	КонецЕсли;
		
	Команда = СобратьКоманду(URL, Данные, Метод);
	
	ВыполнитьКомандуБезопасно(Команда);
	УдалитьАртефактыПослеЗапроса();
	ПрочитатьЗаголовкиОтвета(Метод);
...
Процедура ДобавитьОпциюМетодЗапроса(Опции, Метод)
	...
	Если Метод = МетодЗапросаHEAD() Тогда
		ДобавитьОпциюВСписок(Опции, "--head", Метод);	
	Иначе	
...
Процедура ДобавитьОпциюДанныеЗапросаФайл(Опции, Файл, Метод)
	...
	Если ЭтоHTTP(СхемаURL) И Метод = "POST" Тогда
