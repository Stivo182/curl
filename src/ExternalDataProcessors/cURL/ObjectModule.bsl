...
Перем ОпцииПодерживающиеМногократноеИспользование; // Соответствие
...
// 
// HTTP: Если явно не передан заголовок запроса Content-Type, то MIME-тип определяется 
...
Функция ЭтоОпцияМногократногоИспользования(Опция)
	Возврат ОпцииПодерживающиеМногократноеИспользование().Получить(Опция) <> Неопределено;
КонецФункции

Функция ОпцииПодерживающиеМногократноеИспользование()
	
	Если Не ОпцииПодерживающиеМногократноеИспользование = Неопределено Тогда
		Возврат ОпцииПодерживающиеМногократноеИспользование;
	КонецЕсли;
	
	ПереченьОпций = "-F,--form";
	
	ОпцииПодерживающиеМногократноеИспользование = Новый Соответствие();
	
	Для Каждого Опция Из СтрРазделить(ПереченьОпций, ",|") Цикл
		ОпцииПодерживающиеМногократноеИспользование.Вставить(Опция, Истина);	
	КонецЦикла;
