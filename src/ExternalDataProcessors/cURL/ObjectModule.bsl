...
Функция АутентификацияПрокси(ИмяПользователя, Пароль) Экспорт
	Прокси.Аутентификация = НоваяАутентификацияBasic();
	Прокси.Аутентификация.ИмяПользователя = ИмяПользователя;
	Прокси.Аутентификация.Пароль = Пароль;
	Возврат ЭтотОбъект();
КонецФункции
...
Функция АутентификацияПроксиDigest(ИмяПользователя, Пароль) Экспорт
	Прокси.Аутентификация = НоваяАутентификацияDigest();
	Прокси.Аутентификация.ИмяПользователя = ИмяПользователя;
	Прокси.Аутентификация.Пароль = Пароль;
	Возврат ЭтотОбъект();
КонецФункции
...
Процедура УстановитьНачальныеНастройки()
	...
	ОтправлятьДанныеАутентификацииПриПеренаправленииНаДругойХост = Ложь;
...
//		* Хост - Строка - Хост
//		* Порт - Число - Порт
//		* Аутентификация - см. НоваяАутентификацияBasic
...
Процедура ДобавитьОпциюДанныеЗапроса(Опции, Данные, Метод)
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("Строка") И Не ЭтоАдресВременногоХранилища(Данные) И ФайлСуществует(Данные) Тогда
		ОбработанныеДанные = Новый Файл(Данные);
