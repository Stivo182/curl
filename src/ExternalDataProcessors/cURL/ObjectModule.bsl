...
Функция ОтправитьЗапрос(URL, Метод, Данные = Неопределено)
	...
	Если Не Данные = Неопределено Тогда
		Попытка
			ДобавитьДанныеВПараметрыКоманды(Команда, Данные);
		Исключение
			УдалитьАртефакты();
...
Процедура ДобавитьМетодЗапросаВПараметрыКоманды(Команда, Метод)
	ДобавитьПараметрКоманды(Команда, "--request", Метод);
КонецПроцедуры
...
Процедура ДобавитьЗаголовкиВПараметрыКоманды(Команда)
	...
	Если Не (СхемаURL = "HTTP" Или СхемаURL = "HTTPS") Тогда
		Возврат;
	КонецЕсли;
...
// (HTTP) Добавляет шифры в параметры команды.
// 
// Параметры:
//  Команда - Строка - Команда
...
// Добавляет данные запроса в параметры команды.
// 
// Параметры:
//  Команда - Строка - Команда
//  Данные - Строка - Простой текст, Адрес двоичных данных во временном хранилище
...
//       - Файл
//       - Неопределено
Процедура ДобавитьДанныеВПараметрыКоманды(Команда, Данные)
	...
	Если ТипЗнч(ОбработанныеДанные) = Тип("Файл") Тогда
		...
		Если ОбработанныеДанные.Существует() Тогда
			Если СхемаURL = "HTTP" Или СхемаURL = "HTTPS" Тогда
				ДобавитьПараметрКоманды(Команда, "--data-binary", "@" + ОбработанныеДанные.ПолноеИмя);
			Иначе
				ДобавитьПараметрКоманды(Команда, "--upload-file", "@" + ОбработанныеДанные.ПолноеИмя);
			КонецЕсли;
		Иначе
	...
	ИначеЕсли ТипЗнч(ОбработанныеДанные) = Тип("Строка") Тогда
		...
		ДобавитьПараметрКоманды(Команда, "--data-raw", ОбернутьКавычками(ОбработанныеДанные));
...
// (HTTP) Добавляет файл дампа заголовков в параметры команды.
// 
// Параметры:
//  Команда - Строка - Команда
...
// (HTTP) Добавляет перенаправление запроса в параметры команды.
// 
// Параметры:
//  Команда - Строка - Команда
